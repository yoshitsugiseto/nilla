...
describe('useWebSocket', () => {
  ...
  test('invalidates the expected queries for scoped realtime events', async () => {
    ...
    socket.onmessage({ data: JSON.stringify({ type: 'issue.updated', project_id: 'project-1', workspace_id: 'workspace-1' }) })
    socket.onmessage({ data: JSON.stringify({ type: 'comment.created', issue_id: 'issue-1', workspace_id: 'workspace-1' }) })
    socket.onmessage({ data: JSON.stringify({ type: 'sprint.updated', project_id: 'project-1', workspace_id: 'workspace-1' }) })
    ...
    expect(invalidateQueriesSpy).toHaveBeenCalledWith({ queryKey: ['sprints', 'project-1'] })
