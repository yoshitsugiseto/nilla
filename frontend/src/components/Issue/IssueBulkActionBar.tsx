...
export function BulkUpdateResultPanel({
  ...
  const reasonCounts = result.skipped.reduce<Record<string, number>>((counts, item) => {
          ...
          {Object.keys(reasonCounts).length > 0 && (
            <div className="mt-2 flex flex-wrap gap-2">
              {Object.entries(reasonCounts).map(([reason, count]) => (
                <span
                  key={reason}
                  className="rounded-full bg-white px-2 py-1 text-[11px] font-medium text-amber-800"
                >
                  {reason} {count}件
                </span>
              ))}
