...
| User-facing UI terminology should prefer consistent Japanese labels over mixed English/Japanese copy, while leaving internal identifiers and API/type names in English | The product already uses Japanese explanatory copy, so leaving metrics and workflow nouns half in English makes the interface feel uneven; standardizing visible labels such as dashboard/board/backlog/sprint names, issue actions, reporting terms, and history terminology improves scanability without changing the underlying code model | Keep mixed English labels for product sections and metrics, or translate internal identifiers as well | 2026-03-28 |
| Count-derived drill-down links in reporting cards should disappear when the matching result set is empty | Links like overdue/review/unassigned drill-downs are only useful when there is at least one matching issue; hiding them for `0` avoids dead-end clicks while leaving broader navigation actions, such as opening the whole sprint, intact | Keep every drill-down visible regardless of count, or hide whole cards when any count is zero | 2026-03-28 |
| Trust-oriented settings and terminology polish should avoid cache-only assumptions and fake expansion controls: workspace names should be fetched directly when rendering settings, automation execution logs should page from the server before exposing `さらに表示`, and visible issue/sprint labels should come from shared frontend mappings | The settings page now carries audit and administration meaning on its own, so it should stay correct after hard reloads and should not imply a complete history when only one page was fetched; central label helpers also keep the recent Japanese copy normalization from regressing in dashboard, board, sprint, and issue surfaces | Keep relying on sidebar cache for workspace names, keep the automation log button as a client-side expand-only control, or leave each screen to define its own labels ad hoc | 2026-03-28 |
...
| Date | Changes |
|------|---------|
| 2026-03-28 | Closed the review follow-ups by fetching workspace names directly in `WorkspaceSettings`, turning automation execution logs into offset/limit-backed pagination instead of a fake local expand, and centralizing user-facing issue/sprint labels so Dashboard, Board, Sprint, badges, forms, and history views stay aligned in Japanese |
...
| 2026-03-28 | Refined Dashboard and Sprint reporting cards so zero-count drill-down actions are hidden instead of leading to empty result views, and added frontend coverage for the zero-count cases |
| 2026-03-28 | Standardized user-facing terminology toward Japanese labels across navigation, page titles, issue actions, reporting widgets, history wording, and workspace/project creation forms while keeping internal identifiers and code terms unchanged |
